...
**Version:** 1.2 · 2026-07-26 (v1.0 built D→G; v1.1 folds in the wave-2 review resolutions F1–F4 as in-place "v1.1 amendment" notes on parcels E, F and G; **v1.2** folds in the parcel-G field investigation as one "1.2 amendment" block on parcel G, five fixes T1–T5 — no parcel was added, removed or re-scoped)
...
**v1.2 amendment — THE TANK FILL, five proven defects (2026-07-26, parcel-G field investigation).** The fill shipped, was played, and was found to be a dead-locked offer nobody could complete: a single-gate test that a dropped note locked shut, a between-gate reaction budget that was arithmetically negative at the tempos the game starts at, a middle gate with no cue on it, a "spawn CLOSE" cap that silently did nothing above a middling tempo, and a clank ledger that charged you three penalties for *trying* the figure while declining it outright stayed free. Five edits, all inside parcel G; `fillOnly:false` stays byte-equivalent to current `main` on every one of them.

- **T1 — respace to whole beats.** `fig2:[14,16] → [12,16]` ("4 → 1"), `fig3:[12,14,16] → [8,12,16]` ("3 → 4 → 1"). A tank's own shot flies `k` sixteenths (`k ≤ tank.maxLeadSixteenths`, 4), so the time between *landing* one gate and having to *release* for the next is `budget = (spacing−k)·(15/bpm)s + 2·win`, `win = grooveOpenSec` (0.26s learning → 0.12s expert). At the shipped 2-sixteenth spacing that is **negative** for every `k>2`: `k=4` breaks even only above `15/win` bpm (≈58 learning, ≈125 expert) and `k=3` only above `7.5/win` (≈29 / ≈63) — at the 28bpm start it was −0.55s to −0.83s, i.e. the second note had to be fired before the first one landed. At 4-sixteenth spacing the worst case is `k=4`, where the tempo term vanishes and the budget is exactly `2·win` = **+0.52s learning / +0.24s expert at every tempo**; `k=3` gives +1.06s at 28bpm and +0.34s at 150bpm expert, `k=2` gives +1.59s / +0.44s. Positive across the whole live band (28 → 150, → 172 on a FULL MOON) by construction. Every gate now sits where the whole-beat glow already peaks, and the last gate is still the mercy downbeat, so the finale pop + `tideBloom` payoff is untouched.
- **T2 — unlock.** `fillOpen(tg)` returns the **index of the earliest remaining gate whose window contains this arrival** (searching `j` from `hpMax−hp` upward on the identical `win` test), or `-1`; `handleTankHit` consumes gates `i..j` (`tg.hp -= (j−i+1)`). It shipped testing only the single next gate, so a note you let pass stayed owed forever — its sixteenth was already in the past and no later arrival could match it, and the tank dead-locked into the neutral expiry with every remaining hit unplayable. Now a dropped gate is absorbed by the next one you land, and landing the mercy downbeat always finishes the fill. `hp≤0` still implies `j = hpMax−1`, so the killing blow is always the last gate and the tonic is always the note that is due. The walking bass plays **the landed gate's** note (`step−1` is now that gate's index) — the run may skip a step, which is a run with a note missing, which is what you played.
- **T3 — soft thud.** An off-figure landing on a live fill tank keeps the clank SFX, the spark, the reticle flash, the musical flinch and the beat of your own silence, and drops `state.shots++`, the streak reset and `pushEvent(false)` (a `soft` third argument on `clankShot`; every other call site passes two). Declining the whole offer was already free (v1.1's neutral expiry) while being 40ms out on the one orb you are locked into for the rest of the swell cost three penalties. Only the ledger is spared — the feedback is how you find the sixteenth.
- **T4 — blink cue.** The dead `tank.blinkWin` is revived: the (at most one) live fill tank shapes `TANK_SHELL_MAT` opacity **and** its own shell scale from the distance to its **next needed gate** (`fillOff16` vs `tg.fig[hpMax−hp]`, the same `win` law widened to a floor of `blinkWin` beats so the cue is never narrower than the pocket it advertises), overriding the whole-beat `_openAmt` write for that material. The tank had been wearing the field's glow, so it lit up identically on gates it needed and gates it had already spent — the 3-figure's middle gate was visually dark. With T1 the two clocks mostly coincide, which is the point: they agree on the beats the tank wants and part company on the beats it does not. The husks `tankOpen()` / `tankGlow()` / `tankBeatPhase()` are deleted; `tank.openFrac` stays as a vestigial-marked key.
- **T5 — spawn clamp + Saturn value.** (a) When the fill re-draw's `beatSpawnDist(tank.maxLeadSixteenths)` returns `null` — which above a middling tempo is *every* draw, since a 4-sixteenth flight covers only ~4.9m at 86bpm mid-skill against `rangeNear` 8m — the shipped code silently kept the **first, uncapped** draw (k up to 16), so the swell's fill could be a long-lead orb across the room asking for hits one beat apart. The tank is now placed at the closest distance the live band can hold for that flight (`tankCloseDist()`, no `rnd()`, clamped into `[rangeNear, min(rangeMax, state.range)]`), and if no such distance exists at all the election is dropped and the orb spawns plain — *no tank beats an unreachable tank*, and `_fillSpent8` means that is one silent swell, not a retry. "Tanks spawn CLOSE" is now an invariant instead of a wish. (b) `deal.saturnFillMul` **1.6 → 1.2**, the value its own comment already claimed: at 1.6 the fill draw was `rnd() < min(1, 0.5·1.6) = 0.80`, so a risen SATURN made four in five fills the three-note figure — a rule wearing a weight's clothes. 1.2 → 0.60 against the base 0.50.

### CFG (extend the existing flat `tank` literal)
```
tank:{ ...existing keys..., fillOnly:true, fig2:[12,16], fig3:[8,12,16] }   // 1.2: sixteenths from the peak-final bar's start, one WHOLE BEAT apart; 16 = the mercy downbeat
```
...
- (1.2) No live tempo where the between-gate reaction budget is negative; a dropped gate never dead-locks the fill; an off-figure landing on a live fill tank changes no score, no streak and no accuracy-window entry; a fill tank never spawns beyond the band's near reach.

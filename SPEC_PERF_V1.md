...
| 2026-07-14 | **P3** Load scheduling | Zodiac belt planes still built synchronously, but texture I/O now routed through a bounded, idle-kicked queue (`queueSignArtLoad` / `_signArtPump`) — max **3** concurrent (**2** on LOW) instead of 13 unbounded loads at boot; drains via `requestIdleCallback` so first paint / ENTER isn't blocked. LOW: belt deferred to temple-open (gated `ensureAllSignArt` at dojo-idle sites; temple focus still builds it). Milky confirmed already temple-first (`ensureMilkyShell` only from `updateTempleOrbs` when in-temple; no preload) — no change. `ensureAllSignArt`/`ensureSignArtSlot` names + PNG→JPG soft-fail preserved. Tests 131/131; headless queue check (cap respected, all 13 drain, desktop+LOW) PASS. |
| 2026-07-14 | **P5** Frame budget | **F2** `placeAllSignArt` now dirty-flagged (`_saLastQ`/`_saLastP`) — skips the 13-plane per-frame re-place while sky attitude + camera are static (late texture arrivals still self-place); pure perf, no visual change. **F3** `enhancePlanetTexture` early-returns the raw map on LOW (skips the per-pixel contrast/gamma canvas pass). **F1** new flat CFG lever `signArtLowMode` — `'off'` (default) = glyph-only on LOW, shedding the 13 additive planes; `'always'` = force the belt on LOW. **Desktop belt/globe visuals unchanged** (all guards are `LOW`-only). **F5** confirmed `sky-maps.js SEGMENTS` already tiers shell/globe per LOW/MOBILE — segments not raised. **F4** skipped (optional `?perf=1`): existing `_showFps` HUD already gives frame visibility; avoided a combat hot-loop edit. No combat/beat/projectile timing touched. Tests 131/131; headless F1 truth-table + F2 dirty-flag checks PASS. |

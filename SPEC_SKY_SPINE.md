# The Sky Spine â€” Redesign Wave 3 (every voice is a star; the chorus is the save file)

**Version:** 1.4 Â· 2026-07-26 (1.0 body preserved; wave-3 review resolutions are marked "1.1 amendment" / "1.2 amendment" / "1.3 amendment" / "1.4 amendment" in place)
**Branch:** `redesign/moon-chorus` (continues from waves 1-2, both merged; merge to `main` after user playtest)
**Files touched:** `index.html` (+ regenerated `tools/index-inline.mirror.js` per commit). No new assets, no build, no server, no new fixtures â€” the star data is `fixtures/zodiac_sticks_v1.json`, already shipped and rendered.
**Origin:** 2026-07-24 redesign panel â€” the judges' #1 synthesis: merge "Every Voice Is a Star" with "The Standing Chorus" into one accretion spine. This is the wave that changes why you return.

---

## 0. Product intent

A landed rescue stops being a transient event. Each Echo now calls from the bearing of a REAL star the game already draws â€” one of the ~120 zodiac-band stick stars â€” and returning its voice permanently brightens that star in YOUR sky, in the dojo and the Temple, forever. Every brightened star also contributes a voice to the Standing Chorus: the ambient music of your dojo, playing when you load in, audibly richer for every night you have played. The sky is the save file you see; the chorus is the save file you hear. No counters, no lists, no completion percentages â€” ever.

## 1. Constitutional rules (from the design panel's judges â€” violating these kills the wave)
...
- **Kill-switches:** `CFG.stars.on` and `CFG.chorus.on` â€” each `false` restores today's behavior exactly (hot call-sites read the raw boolean first, house style). Waves 1-2 hard constraints (rhythm-safe, trainer/temple mechanics inertness, no new UI/toasts/strings, flat CFG literals, mirror regen per commit, gitnexus impact before edits, tests 133/133) all inherit.

## 2. Existing substrate (read before designing)

- `fixtures/zodiac_sticks_v1.json`: 13 figures, â‰¤120 stars as `[lon_deg, lat_deg]` approximate J2000 ecliptic; edges index per-figure star lists. Find the loader + renderer in `index.html` (search `zodiac_sticks` / `signArt` / stick draw) â€” it already converts eclipticâ†’horizontal per frame for the clocked sky, which is the transform this wave reuses.
...
## 3. Parcel H â€” THE LIT SKY (catalog identity + persistence + rendering)

### Design
- **Identity:** each stick star gets a stable id `"<figureKey>:<starIndex>"` derived from the fixture (no fixture edits).
- **State:** `starChorus = { [id]: level }`, level 1..`stars.levels` (5). Loaded at boot from `localStorage['aimdojo.starChorus']`, saved on change (throttled). Accretion-only: level never decreases; malformed storage â†’ empty (never throws).
- **Rendering:** on the existing stick-star draw path, a recovered star renders brighter/larger by level â€” scale the existing point/vertex brightness by `1 + level*stars.glowStep`, with a soft warm tint at full level. Visible in BOTH dojo and Temple skies (the Temple is the trophy room; this is sky content, not a run mechanic). `reduceMotion` unaffected (static brightness, no pulsing â€” do not add pulsing for anyone).
- **Kill-switch:** `stars.on:false` â†’ sticks render exactly as today; storage untouched.
...
### 1.1 amendment â€” storage is untrusted input, and ids must be catalog-backed

Wave-3 review found the loader would accept any object (arrays included), any version, and any key â€” so a hand-edited or foreign `aimdojo.starChorus` could seed ids the catalog does not contain, and those phantoms would then be ranked and **sung** by the Parcel J chorus. Clarification, in force from 1.1:
...
## 4. Parcel I â€” STAR-BOUND SPAWNS + THE VOICE FLIES HOME

### Design
- **Spawn binding:** when `stars.on` (post-graduation, free-play, not temple), `spawnTarget` picks the Echo's AZIMUTH from a real star: compute risen stick stars (altitude > `stars.minAltDeg`) via the existing eclipticâ†’horizontal transform; prefer stars not yet at full level; respect the existing `spawnMinDeg`-from-aim constraint (if the chosen star violates it, pick another; if none qualify, fall back to today's random direction â€” silent, no seam). Elevation/pitch and DISTANCE laws untouched (direction only). Selection uses `rnd()` freely (free-play only). Store `tg.starId`.
- **Voice return:** on a landed scoring kill of a star-bound Echo, during the NEXT beat gap (never inside an open window): a thin, low-alpha line traces from the burst position toward the star's true current sky position over ~one beat, fades, and the star ticks +1 level (persisted). Reuse an existing line/trail resource â€” no new material class if avoidable. `reduceMotion`: no flight line; the star simply brightens on the next beat.
- **Multi-kill beats:** volleys can return two voices in one gap â€” stagger the lines by a sixteenth; both tick.
- **Tank/gold/speed/mover:** all kinds may be star-bound (the star is a bearing, not a kind).
- **Kill-switch:** `stars.on:false` â†’ spawn path byte-identical to today (the fallback IS today's path).
...
### 1.1 amendment â€” stream purity by construction, and the gap is the law

Three review findings, all of the same shape: the parcel was reaching into machinery that is not its own.

**(a) Spawn stream purity.** 1.0 said "Selection uses `rnd()` freely". That is withdrawn. Rolling the pitch early and drawing the star from `rnd()` shifted the shared spawn stream, so `beatSpawnDist`, the drift velocity, the kind roll and the tank roll all landed on different numbers than the no-stars build â€” the "fallback IS today's path" guarantee was false. In force from 1.1:
...
- **No transport = no windows.** With the Transport stopped (sound off, a graph that never started) or `grooveVuln` off, the gap is *now* â€” which also repairs the pathological case where a stopped Transport pinned `_openAmt` at 1 and the freeze was permanent.

### 1.3 amendment â€” the tick is applied at the drain, and a flight record owes nothing

Round-3 review found the 1.2 drain only *half* left the renderer's hands. A due return that got a line was moved into `_starFly` with its level still unpaid, and the payment happened at `starFlyRetire` â€” which lives **below** the open-window freeze. So the LAW-level tick was back under the frame's control after all: at a pathological render cadence, or with a stale-stuck `starWinOpen()`, a return could be drained and still never ticked. In force from 1.3:

- **`starFlyDrain` applies the tick, unconditionally, at drain time.** Every return whose stamp has passed is granted right there â€” line or no line, window or no window â€” alongside the debt payment that already ticked in place. Nothing downstream of the drain owes a level.
...
- **Exactly one tick per return, structurally.** A return is in exactly one of three states: queued and unpaid (`_starPend`), displaced and unpaid (`_starDebt`), or paid (dropped, or carried as a flight). `starLitGain` therefore has exactly four reachable call sites â€” debt and ring inside `starFlyDrain`, debt and ring inside `starFlyClear`'s teardown â€” and the teardown retires airborne flights *without* granting, because they were paid when they launched. `reduceMotion` and the line-cap arms are the same single call as every other return.

### 1.4 amendment â€” the block is the orb's true silhouette, and a refused gesture is audible

Parcel I put the Echoes on the zodiac band on purpose: a bearing that means something is a bearing a star already occupies. The unintended consequence is that the field now stands **exactly where the sky picks are**, and the held-E arbitration was never re-measured against that. Field investigation found the Listen gesture failing far more often than the sky was actually covered, and failing without a word. Four resolutions, in force from 1.4:

- **(a) The pad is a skim margin, not authority.** `orbBlockPad:2.7` plus `orbBlockPx:52` of screen pad grew each Echo a **22-27Â° exclusion cone** â€” several times the sphere the player can see â€” so a live field shadowed **15.8% of the dome** and cancelled **41.2%** of the bearings a pick could have come from. The player was refused by orbs that were merely *near* the star, which from the chair is indistinguishable from the key not working. In force: `orbBlockPad:1.15` (a thin skim on the true radius, so a sphere one pixel off the reticle is still not shot through) and `orbBlockPx:0` â€” the projected disc IS the drawn disc. Measured result: **1.45% of the dome, 8.6% of picks**. The block now answers one question only: *is this Echo in front of that star?* The two inline `!=null` fallbacks in `_lsnOrbBlocksSky` carry the same defaults, so a stripped CFG behaves as the shipped one.
- **(b) `combatPx` is vestigial and stays annotated.** Its near-miss combat-priority clause was already removed when star-bound spawns landed. 1.4 records *why it was never load-bearing at the low end*: the orb radius lerps `radSlow 1.15 â†’ 0.62` with `diffT`, so the slowest night carries the **largest** orb, and at every distance the beat-quantized band can hold there the padded disc already reads wider than 88px. **The clause could never fire below ~59.5bpm** (crossover verified against the shipped ballistic band and orb-radius law); it only ever had room at tempi where the orb had shrunk and moved out. The constant is kept, named and annotated, so a number that once meant something is not silently reused for something else.
- **(c) A refused gesture speaks.** Held-E and an ordinary shot are the same physical press; a silent refusal therefore reads as a broken key, and the player has no way to learn the rule. `skyListenTry` now names both failures in the **existing ghost-toast channel**: the Echo block says `AN ECHO IS IN THE WAY` (`skyBlockedByEcho`), and a held-E press on bare sky says `NOTHING IN THE SKY THERE` (`skyNoMark`) before it clears the mark. These are **gesture-failure feedback, not narration** â€” they fire only on an explicit held-E press, never per kill and never on a session boundary â€” so wave-2's session-boundary text budget and the "exactly one Bow toast" rule are untouched, and the story spec's no-toast-spam constraint holds. Both keys ship with JA in the story voice, â‰¤14 glyphs for the nowrap toast.
- **(d) The gesture is consumed by success, not by attempt.** `fire()` set `_skySelectUsed` **before** asking whether the Listen happened, so a blocked held-E fire burned the whole press: the shot went to combat *and* the E-release went inert, losing an entry the player had already earned on an existing mark. In force: `_skySelectUsed` is set only when `skyListenTry()` returns true. A refused held-E fire is an ordinary shot and nothing else. The two-step entry is unchanged â€” a successful select still marks the press used so the release cannot immediately enter, and entry still requires a live `_skySel`.

### Acceptance
- Beat-quantized distance, open-window timing, grading, scoring: untouched (verify by diff read â€” the star contributes ONLY the azimuth used where the old random azimuth was used).
...
- (1.3) A return that drains into a flight has already ticked; freezing every subsequent frame (or pinning `starWinOpen()` true forever) loses no level. No path grants the same return twice â€” including a teardown that happens mid-flight.
- (1.4) An Echo blocks a Listen only where it visibly covers the star; a held-E aimed a full orb-width clear of any sphere always reaches the sky.
- (1.4) Every held-E refusal produces exactly one toast; no other press, kill, spawn or session boundary produces any new string.
- (1.4) A blocked held-E fire launches an ordinary projectile AND leaves the E-release able to enter the Temple on an existing mark; a successful held-E select still leaves the release inert, and entry still requires `_skySel`.

## 5. Parcel J â€” THE STANDING CHORUS (the save file you hear)

### Design
- **Stems:** each recovered star (level â‰¥ 1) can sing: a soft sustained voice on the existing `pad`/`arp` infrastructure â€” pitch mapped from the star's figure + index into the active THEME's PENTA (octave spread by level: higher level = an added octave shimmer at most). NO new synth if `pad` + one existing voice can carry it; if one dedicated shared PolySynth is genuinely needed, it is the single sanctioned addition (justify in the CFG comment, keep it â‰¤ `chorus.maxStems` polyphony).
- **Tonight's ensemble:** a deterministic subset of lit stars sized â‰¤ `chorus.maxStems` (8), rotated by local date (e.g. hash(date + id) sort) â€” so a 40-star sky sings a different octet each night. Preference: risen stars first (the chorus you hear is overhead â€” the sky and the sound agree).
- **When it sings:** (1) the start/pause overlay (menu ambience â€” this is the "boot chorus": one stem fades in per second, richer with history); (2) mercy bars â€” stems swell with the existing `tideBloom`; (3) the Bow's HOLD â€” the chorus holds under the Mandala. NOWHERE else. During active play the stems are hard-silent (gain 0, not just quiet).
- **A new voice joins:** when a star first reaches level 1 mid-run, its stem is added to the ensemble pool immediately â€” the player hears it for the first time in the next mercy bar (or the Bow): growth is heard within the same session it happened.
- **Kill-switch:** `chorus.on:false` â†’ no stems, menu/mercy/Bow audio exactly as today.
...
### 1.1 amendment â€” the lifecycle, not just the call sites
...
- **The mercy gate hard-closes at the mercyâ†’rise boundary.** 1.0 gave every moment `CHORUS_TAIL_SEC` (2.5 s) of slack past its stated length so the stems' own release would not be chopped. For the mercy bar that slack lands squarely inside the next swell: the chord is struck with a length of one bar, so its release *begins* where the rise begins, and up to 2.5 s of chorus was audible under live combat. "Hard-silent during active combat" is literal â€” **zero audible chorus energy once the rise's first beat sounds**. The tide's existing mercyâ†’rise latch in `onGrid` (the same one that steps the BPM â€” no second clock) now ramps the chorus bus to silence over â‰¤120 ms and cancels the pending tail outright. The tail slack survives for the overlay and the Bow, which are the only places it was ever the player's.
- **Polyphony is EXACTLY `chorus.maxStems`.** 1.0 set `maxPolyphony = maxStems*2` as "release-tail headroom for the one handover the game has". That headroom is not headroom, it is the sixteen-stem pile the cap exists to forbid: a handover put a fresh ensemble on top of eight still-releasing ones and all sixteen sounded. The cap is now the stated number. **Consequence the implementation must own:** Tone's `PolySynth` does not voice-steal â€” past `maxPolyphony` it *drops* the note, and a released voice does not rejoin the pool until its oscillator stops one full release later. A hard cap therefore only works if a replacing moment takes the pool back first (a short borrowed release), and waits for it before striking. A scheduled moment never waits â€” the mercy downbeat outranks the chorus. The audible cost is a fraction of a second between an outgoing and an incoming ensemble, on the overlay and the Bow only; that is the accepted artifact.
...
### 1.2 amendment â€” nothing sings before the catalog binds

1.1 closed the phantom-id hole with `starLitBind`'s prune, but `chorusPick` kept a pre-fixture arm: when the sky had not landed yet it ranked **everything** in `_starLit` by pure hash order. So a payload like `{v:1,lv:{"fake:1":1}}` sang on the start overlay before the idle fixture fetch completed â€” or forever, if that fetch failed. Ranking cannot be the guard, because the thing being guarded against is exactly the id ranking has no opinion about. In force from 1.2:

- **The pre-fixture fallback arm is removed.** Until `starLitBind` has run against a real fixture, the ensemble is **empty** and `chorusPick` returns 0. The pick also skips, locally, any id with no vertex â€” the guarantee belongs to the pick, not borrowed from another function.
...
### 1.3 amendment â€” the entrance step is `menuFadeSec`, exactly

The chain waited `menuFadeSec âˆ’ CHORUS_LEAD_SEC` and then scheduled `CHORUS_LEAD_SEC` ahead of its own fresh `Tone.now()`. Those two do not cancel â€” every note already carried the lead â€” so the **emitted** attack times were `menuFadeSec âˆ’ 0.05` apart: 950 ms at the shipped 1.0 s. In force from 1.3: the callback waits the full `menuFadeSec` and the same constant lead (zero, for a held moment) is added to every note, so note-to-note spacing is exactly `menuFadeSec`.
...
Sequential H â†’ I â†’ J (one commit each, Sonnet verify between, gitnexus impact before edits), then Codex read-only review of the wave-3 diff, fix rounds to GREEN, push branch for user playtest. Feel values are first guesses; the user tunes by ear/eye.
...
- I: does the voice-flight in the beat gap feel like ceremony or clutter? (`lineAlpha`, `lineBeats`) Does star-bearing spawning change how the field feels (bearings cluster along the ecliptic band â€” good "the sky is calling" or too directional?)
- J: load the game the morning after a session â€” does the boot chorus land as "my history is singing"? Is `maxStems:8` rich or muddy?
- The big one: after two evenings, do you feel the pull the panel promised â€” "my sky will be one voice brighter tonight"?

...
function wallOptions(overrides = {}) {
  return { markGlyph: true, terrainOn: true, terrainAmp: 1, curveBite: 2.2, wallsOn: true, wallDissolve: 95, wallGlow: 1, wallSat: 1, wallPalette: null, ...overrides };
}
...
test("Enfilade rear slots crumble before recycle and reduced-motion identities stay pinned", () => {
  const vertexContext = vm.createContext({
    ML_ARCH_N: 11, ML_ARCH_EVERY: 4, ML_BITE: false, ML_TERRAIN: false, ML_WALL_REAR0: -12, ML_WALL_REAR1: -8, ROAD_MPB: 27, ROAD_FADE0: 734.4, ROAD_FADE1: 864,
    _roadG: (number) => (+number).toFixed(5),
  });
  ...
  const identities = (reduced) => {
    ...
    const context = vm.createContext({
      Math, Number, Float32Array, ML_WALLS: true, ML_NAVE: true, ML_WALL_N: 7, ML_ARCH_N: 11, ML_ARCH_EVERY: 4, ML_ARCH_BEHIND: 8, LOW: false, reduceMotion: reduced,

...
function inverseFlags(source, options) {
  const names = ["ML_WALLS", "ML_WALL_EXHALE", "ML_WALL_ECHO", "ML_MERCY_INVERSE"];
  ...
  vm.runInContext(`${declarations}\nthis.flags={walls:ML_WALLS,exhale:ML_WALL_EXHALE,echo:ML_WALL_ECHO,inverse:ML_MERCY_INVERSE};`, context);
...
function emittedWallFamily(source, options, { low = false, reduced = false } = {}) {
  ...
  const context = vm.createContext({
    ...
    CFG: { moonline: options }, LOW: low, reduceMotion: reduced, ML_TERRAIN: true, ML_BITE: true, ML_WALL_STAR: false, ML_WALL_EXHALE: flags.exhale, ML_WALL_ECHO: flags.echo, ML_MERCY_INVERSE: flags.inverse,
...
test("Inverse kill-switch preserves every shipped reveal combination and remains independently wired", () => {
  const assertGateContract = (source) => {
    const matrix = [
      [inverseOptions({ mercyInverse: true, wallExhale: 1, wallEcho: true }), { walls: true, exhale: 1, echo: true, inverse: true }],
      [inverseOptions({ mercyInverse: false, wallExhale: 1, wallEcho: true }), { walls: true, exhale: 1, echo: true, inverse: false }],
      [inverseOptions({ mercyInverse: true, wallExhale: 0, wallEcho: true }), { walls: true, exhale: 0, echo: true, inverse: true }],
      [inverseOptions({ mercyInverse: true, wallExhale: 1, wallEcho: false }), { walls: true, exhale: 1, echo: false, inverse: true }],
      [inverseOptions({ mercyInverse: true, wallsOn: false }), { walls: false, exhale: 0, echo: false, inverse: false }],
    ];
  ...
  assert.deepEqual(inverseFlags(mutation, inverseOptions({ mercyInverse: false, wallEcho: false })), { walls: true, exhale: 1, echo: false, inverse: false }, "the mutation survives a both-off-only check");

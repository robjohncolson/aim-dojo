...
- **Kill-switches:** each parcel has a single boolean in CFG (`tide.on`, `quietTick.on`, `bow.on`). With all three `false`, runtime behavior must be equivalent to today's (every added term multiplies by 1 / adds 0 / early-returns).
...
- **Sky Temple untouched:** none of the three systems run while `templeActive`.
...
### CFG
```
bow:{ on:true, holsterBeats:8, holsterMinSec:12, cancelGraceSec:1.5, riseBeatsPerOrb:1,
      ritBars:2, mandalaSec:4, mandalaMaxDots:60, mandalaArcDeg:90, lineHoldSec:4 }
```

...
5. If `thread_id` provided and mismatches day thread → 409 or ignore and use day thread (prefer: **ignore client thread_id if wrong date; always bind to day thread**).  
...
- Full essay body unless as a **one-line** optional `day_headline` if essay ready (not full essay paste in v1 — keep context small)

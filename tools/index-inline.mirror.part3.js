...
function drawWasdLane(){
  ...
  const laneCue=!roadLive();   // THE STAR ROAD subsumes the NOTE LANE: the sequence it drew flat at the crosshair is now sequenced ahead of you in the world, so the centre letter stands down while the road owns the floor. The opt-in BEAT CIRCLE (wasdHud) is deliberately untouched — it is the player's own training wheel, not the lane. roadLive() reads the raw kill-switch first, so road.on:false makes laneCue true and both expressions below are the shipped ones verbatim
  const on = !templeActive && !MOBILE && CFG.wasdRhythm && CFG.beatQuant && toneReady && state.running && Tone.Transport.state==='started' && (CFG.wasdHud || (CFG.wasdTapText && !laneCue) || ((CFG.wasdLetter || reduceMotion) && laneCue));   // THE STAR ROAD: the letter clause is the only one the road takes over; the tapText clause exists so an explicitly opted-in TAP TIMING READOUT is not collateral damage of the lane standing down (it used to reach the canvas by riding wasdLetter's default). With laneCue true it is `wasdTapText && false` — a dead term in an ||, so the road.on:false expression is character-for-character the shipped one. canvas shows if ANY center cue is wanted; wasdHud gates the circle, wasdLetter the letter; hidden on touch (no keys to answer it with)

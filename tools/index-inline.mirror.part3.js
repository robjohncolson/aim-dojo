...
function drawWasdLane(){
  ...
  const laneCue=!(roadLive() && ROAD_LANE_READY);   // THE STAR ROAD subsumes the NOTE LANE — but only when it CARRIES it: the centre letter stands down the moment the road tints its bands by required lane and draws the glyph mid-band (ROAD_LANE_READY, index.html:1641), never before, so the sequence is MOVED into the world rather than deleted from it. The opt-in BEAT CIRCLE (wasdHud) is deliberately untouched — it is the player's own training wheel, not the lane. Both gates below fall back to the shipped expressions verbatim whenever laneCue is true, which today is always (THE RIVER carries no lane channel) and stays always under the raw kill-switch road.on:false
  const on = !templeActive && !MOBILE && CFG.wasdRhythm && CFG.beatQuant && toneReady && state.running && Tone.Transport.state==='started' && (CFG.wasdHud || (CFG.wasdTapText && !laneCue) || ((CFG.wasdLetter || reduceMotion) && laneCue));   // THE STAR ROAD: the letter clause is the only one the road will ever take over; the tapText clause exists so an explicitly opted-in TAP TIMING READOUT is not collateral damage of the day the lane stands down (it reaches the canvas today by riding wasdLetter's default). With laneCue true — every shipped configuration until THE BANDS lands, and every configuration at all under road.on:false — it is `wasdTapText && false`, a dead term in an ||, so this reduces character-for-character to the shipped expression. canvas shows if ANY center cue is wanted; wasdHud gates the circle, wasdLetter the letter; hidden on touch (no keys to answer it with)

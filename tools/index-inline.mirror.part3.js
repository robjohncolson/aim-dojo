...
function enterRunning(){
  ...
  if(_themeReveal){ _themeReveal=false; if(!trainMode){ flashTheme(); if(CFG.deal.on){ const dl=dealLine(); if(dl) try{ showGhostToast(dl); }catch(e){} } } else showGhostToast(T('trainerToast','MOONLINE TRAINING')); }   // full runs name the song; trainer announces itself — and THE SKY DEALS THE NIGHT rides the same threshold moment on the same surface the trainer already speaks from (the song takes the flash, the sky takes the toast; no new surface, no second moment). Exactly one line, and it is the WHOLE text budget of the parcel

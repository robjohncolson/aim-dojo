(function(){ // GENERATED by tools/extract-inline.mjs — line-preserving JS mirror of aim-dojo-main.js, part 2/5. Line N here = aim-dojo-main.js line N. Do not edit.
...
function buildRoadDust(){
  ...
  roadDustMat=new THREE.ShaderMaterial({ transparent:true, depthWrite:false, fog:false, blending:THREE.AdditiveBlending, premultipliedAlpha:true,   // …and THE ONE WORD (wave 8.1): the fragment below has always emitted vec4(uCol*a, a), which IS premultiplied, but three.js r128's Material defaults this flag to false and setBlending then picks gl.blendFunc(SRC_ALPHA, ONE) — delivering uCol·a². Saying so picks (ONE, ONE) and the layer arrives at the alpha it was written for: peak 0.4675 instead of 0.2186, i.e. +83.7/255 green against a road rail's +93.5/255 (90% of a rail at its brightest, 70% on average) instead of 42% and 26%. Nothing else on the road is touched: the ribbon and the arches keep the compositing they shipped with
    ...
    uniforms:{ uNow:U.uNow, uCol:U.uInk, uNaveGold:U.uNaveGold, uTerrain:U.uTerrain, uTerrainBase:U.uTerrainBase, uHorizon:U.uHorizon,   // uInk, clock, palette and terrain profile are the road's OWN objects; no dust-only frame write exists
    ...
    vertexShader:[
      ...
      ...(ML_TERRAIN?['  float tv=terrainVis(u,P.x,P.y); P.y+=cyAt(u);']:[]),   // terrainVis height is deck-local before cyAt lifts the mote; P.y is already the mote's local height above that deck
      ...
      '  vA=uAmt*uDustGlow*(0.55+0.45*aSeed)*smoothstep('+_roadG(-ML_DUST_BEH_M)+','+_roadG(-ML_DUST_BEH_M*0.45)+',u)*(1.0-smoothstep('+_roadG(ML_DUST_FAR0)+','+_roadG(ML_DUST_FAR1)+',u))'+(ML_TERRAIN?'*tv':'')+';',   // both ends of the window are ramps, so the wrap has no seam; the terrain branch adds the shared brow cut and nothing on the CPU
      ...(ML_NAVE ? ['  vSeed=aSeed;'] : []),
      '}'
    ].join('\n'),
    fragmentShader:[
      ...
      '  if(a<=0.004) discard;',
      ...(ML_NAVE ? ['  vec3 pollen=mix(vec3(1.0,0.859,0.522),vec3(1.0,0.45,0.12),step(0.92,vSeed)); vec3 col=mix(uCol,pollen,uNaveGold);','  gl_FragColor=vec4(col*a, a);'] : ['  gl_FragColor=vec4(uCol*a, a);']),
      '}'
    ].join('\n') });
...
function roadArchFill(n0){
  ...
  if(ML_NAVE && AU.uVeil) AU.uVeil.value=LOW?0:Math.max(0,Math.min(1,+M.naveVeil||0));
...
(function buildRoad(){
  ...
  roadMat=new THREE.ShaderMaterial({ transparent:true, depthWrite:false, fog:false, ...(ML_MARK?{blending:THREE.CustomBlending,blendEquation:THREE.AddEquation,blendSrc:THREE.OneFactor,blendDst:THREE.OneMinusSrcAlphaFactor,blendEquationAlpha:THREE.AddEquation,blendSrcAlpha:THREE.OneFactor,blendDstAlpha:THREE.OneMinusSrcAlphaFactor}:{blending:THREE.AdditiveBlending}), extensions:ML_MARK&&!LOW?{derivatives:true}:undefined,   // ROUND 4: one custom-blended mark pass is algebraically the old normal-blended dark socket followed by the additive street: premultiplied source light + destination*(1-socketA). This removes the second displaced-ribbon draw without changing the socket, inlay or unmarked additive road
    ...
    vertexShader:(ML_TERRAIN?[
      ...
      roadTerrainShader(),
      (LOW
        ? 'void main(){ vec4 wp=modelMatrix*vec4(position,1.0); vXZ=wp.xz; float u=-wp.z; vTerrainVis=terrainVis(u,0.0,0.0); wp.y+=cyAt(u); gl_Position=projectionMatrix*viewMatrix*wp; }'
        : 'void main(){ vec4 wp=modelMatrix*vec4(position,1.0); vXZ=wp.xz; float u=-wp.z,b=uNow+u/'+_roadG(ROAD_MPB)+'; vec3 sc=sin(uW*b+uP); float cx=dot(uA,sc)'+(ML_BITE?'+uBite.x*sin(uBite.y*b+uBite.z)':'')+'-uBase.x-uBase.y*(b-uNow); vTerrainVis=terrainVis(u,cx,0.0); wp.y+=cyAt(u); gl_Position=projectionMatrix*viewMatrix*wp; }')
    ...
    fragmentShader:(ML_RIBBON?[
      ...
      ...(ML_TERRAIN ? ['  if(vTerrainVis<=0.004) discard;'] : []),                                      // ROUND 4: an occluded fold used to run the entire grid + mark SDF (twice, with the socket pass) before multiplying visibility into final alpha; reject it before even the course/texture reads
      ...
      (LOW ? ('  float cx=uA.x*sin(uW.x*b+uP.x)'+(ML_BITE?'+uBite.x*sin(uBite.y*b+uBite.z)':'')+'-uBase.x-uBase.y*(b-uNow);')
           : ('  vec3 sc=sin(uW*b+uP); float cx=dot(uA,sc)'+(ML_BITE?'+uBite.x*sin(uBite.y*b+uBite.z)':'')+'-uBase.x-uBase.y*(b-uNow);')),                    // the RE-BASED centreline: 0 value and the selected residual slope at the now-line; every presentation reads this same expression
      ...
      ...(ML_MARK?[
      '  float markS=markStretch(d);',
      '  vec2 markQ=vec2((vXZ.x-cx)/'+_roadG(ROAD_MARK_W)+',(fract(b)-0.5)*'+_roadG(ROAD_MPB)+'/'+_roadG(ROAD_MARK_W)+'/markS), markP=markTurn(markQ,lane);',
      '  float markDc=markChev(markP), markD=max(markDc-'+_roadG(ROAD_MARK_T)+',abs(markP.x)-1.03), markAA='+(LOW?'0.05500':'clamp(fwidth(markD),0.006,0.20)')+';',
      '  float markInside=1.0-smoothstep(-markAA,markAA,markD), markGroove=(1.0-smoothstep('+_roadG(ROAD_MARK_G)+'-markAA,'+_roadG(ROAD_MARK_G)+'+markAA,markD))*step(0.0,markD);',
      '  float markCore=1.0-clamp(markDc/'+_roadG(ROAD_MARK_T)+',0.0,1.0); markCore=markCore*markCore*(3.0-2.0*markCore);',
      ...
      // ---- THE BREATH (wave 8.1 · SPEC_MOONLINE §1.1): the pre-wave-7 FLOOR WASH, restored on the biggest surface a
      //      floorless world has. ONE multiply on the ink the rails, the crossbars, the cell fills and the now-line are
      //      ALL already summed into, so the whole ribbon swells toward the beat and releases after it as one body —
      //      which is what the wash did, and why the player could feel the beat coming without reading anything. It
      //      touches `ink` and never `col` or the alpha: the hue still means LANE, the fade still means DISTANCE, and
      //      the KEY's own colour half of the old law lives at the crosshair (parcel W) where a letter can carry it.
      //      uBreath is 0 on every frame the cue is not owed (see roadBreath), so this line then multiplies by 1.0.
      '  ink*=1.0+uBreath;',
      ...(ML_NAVE ? [
      ...
      ] : (ML_TERRAIN ? ['  gl_FragColor=vec4(mix(col*ink,nave,uNaveGold),outer*fade*uAmt*vTerrainVis);'] : ['  gl_FragColor=vec4(mix(col*ink,nave,uNaveGold),outer*fade*uAmt);']))
      ] : (ML_TERRAIN ? ['  gl_FragColor=vec4(col*ink, outer*fade*uAmt*vTerrainVis);'] : ['  gl_FragColor=vec4(col*ink, outer*fade*uAmt);'])),
      '}'
    ]:[
  ...
  roadMesh=new THREE.Mesh(ML_TERRAIN?roadTerrainGeometry():new THREE.PlaneGeometry(ROAD_PLANE_W, ROAD_PLANE_L), roadMat);
  if(!ML_TERRAIN) roadMesh.rotation.x=-Math.PI/2;                         // the kill-switch keeps Wave 9's rotated two-triangle plane; the terrain strip is authored directly in XZ so u remains exactly −z
...
function roadBreath(r){
  /* THE BREATH (wave 8.1 · SPEC_MOONLINE §1.1) — THE PRE-WAVE-7 FLOOR WASH, RESTORED ON THE RIBBON.
     Before wave 7 the whole playing field swelled in saturation INTO each beat and released after it (e4faf65, "the
     in-focus key's colour washes the floor on the beat"), and that swell — not the letter, not the ring — is how the
     player felt when to release. Wave 7 hid the surfaces it washed under the road; wave 8 removed the room entirely.
     What comes back here is the SAME CURVE on the world's biggest surface, and nothing else about it is new:

       THE CURVE is beatSwell(), the one copy of the envelope the floor and the crosshair already share — no second
       shape, no reinvention. Amplitude is CFG.moonline.breathMax, whose default IS the old CFG.floorBeatMax 0.45.

       THE GATE is wasdBeatCueOn(), the shipped floor-beat gate exactly (templeActive · MOBILE · CFG.floorBeat ·
       wasdRhythm · beatQuant · running · toneReady · Transport started · (!reduceMotion || trainMode)). roadSync only
       reaches this line when roadLive() is true, and roadLive() already excludes trainMode — so the trailing clause
       reduces to !reduceMotion and the REDUCED-MOTION STANCE IS INHERITED VERBATIM: the pre-wave-7 wash was OFF in free
       play under reduced motion (only the trainer ever kept a discrete flash), so the ribbon does not breathe there
       either, and a reduced-motion player loses nothing they ever had. MOBILE is inherited for the same reason.
       The trainer's own floor keeps its floorBeat untouched: updateFloorBeat still asks !roadLive() and still calls
       wasdBeatGlow(), and not one character of either changed this wave.

       THE CLOCK is the road's own r = roadBeatNow(), and that is the identity this parcel owes ONE CLOCK. Expand it:
         roadBeatNow()  = wasdBeatsHeard() + freeze            (freeze = grooveGroove ? grooveFreezePhase : 0)
                        = (wasdBeats() − audioLat()/bps) + freeze
                        = ((ticks/PPQ − freeze) − audioLat()/bps) + freeze
                        = ticks/PPQ − audioLat()/bps.
       So r is the RAW whole beat, latency-corrected — which is character-for-character the clock the pre-wave-7 wash
       used (`beats = Tone.Transport.ticks/Tone.Transport.PPQ`, e4faf65, before grooveFreezePhase existed) plus the one
       correction every cue in this game now carries. Two consequences, both wanted:
         · round(r) is the instant the ribbon's own cell boundary crosses the now-line — the shader's beat at u metres
           is b = uNow + u/ROAD_MPB, so b is integral at u = 0 exactly when r is. The surface and the swell painted on
           it can NEVER disagree about which beat it is; there is no second clock on the road.
         · round(r) is also the centre of the ARRIVAL window (grooveOpenSec is ±seconds around the heard whole beat),
           so the peak of the breath is the moment to LAND a shot — "the correct fire time" the swell used to gauge.
       The crosshair's letter keeps its own clock on purpose: wasdBeatGlow() reads wasdBeats(), which is the raw beat
       minus grooveFreezePhase 0.5, so the LETTER peaks on the "and" (where the WASD tap is due) while the ROAD peaks on
       the "one" (where the shot is due). Two cues, two actions, one envelope — not two clocks for one event.
     ZERO GAMEPLAY MATH: this function is a pure read. It writes nothing, it is called from nowhere but roadSync's
     per-frame uniform write, and the treadmill law, the wake, the epoch and every judging path are untouched. */
...
function roadSync(){
  ...
  if(live!==_roadVis){                                                    // ONE LATCH, on the road's own visibility — so everything below costs one boolean compare on an ordinary frame
    ...
    if(roadSocket) roadSocket.visible=live;
...
/* ===== STAR-BOUND SPAWNS + THE VOICE FLIES HOME (wave 3 parcel I) =====
   [WAVE 8.2, Y2b: the ceremony's LOOK changed and nothing else did. The flight line is now a golden PULSE riding the
   star-tether's own path instead of a pale line drawing itself along it — see starFlyStep's draw block. Every law in
   this header (the gap stamp, the drain, the freeze, the debt, where the level is PAID) is byte-identical, and the
   grep that proves it is that no function above starFlyStep's mesh writes was touched by that wave at all.]
...
function starFlyStep(dt){
  ...
  for(let k=_starFly.length-1;k>=0;k--){
    ...
    if(!m){ m=f.mesh=newTrailMesh(); m.position.set(0,0,0); m.visible=true;   // the trail pool's own additive, depth-test-free THREE.Line — no new material, no new geometry class. World coords, so the mesh sits at the origin instead of on the player. Born visible because a launch only ever happens in a gap
      ...
      ca[0]=ML_TETH_RGB[0]*ML_TETH_PULSE_TAIL; ca[1]=ML_TETH_RGB[1]*ML_TETH_PULSE_TAIL; ca[2]=ML_TETH_RGB[2]*ML_TETH_PULSE_TAIL;
      ca[3]=ML_TETH_RGB[0]; ca[4]=ML_TETH_RGB[1]; ca[5]=ML_TETH_RGB[2]; m.geometry.attributes.color.needsUpdate=true;   // THE PACKET'S OWN RAMP (wave 8.2, Y2b), written ONCE at acquire exactly as the pale-white one was: dim gold tail → full ML_GOLD head, so the thing travelling up the thread has a front and a back and the eye can tell which way it is going
    ...
    const e0=Math.max(0,e-ML_TETH_PULSE);                                // …and the packet's TAIL, ML_TETH_PULSE of the path behind its head. THE RETURN RIDES THE THREAD (wave 8.2, Y2b): this line used to run from the burst point to the head, growing to the full orb→star path, which is a line being drawn. It is now a short bright PULSE travelling from the burst point up the tether to the star — one line geometry, a moving brightness — over the SAME f.life the ceremony always had. Both ends are the tether's own two ends by construction (f.from is where the orb was, starWorldAt is where the star is), so the packet cannot leave the thread it is riding. VISUAL ONLY: not one line of the gap law, the epoch machinery or the level tick above is touched by this — starFlyDrain still pays every due return, unconditionally, before anything here draws
    ...
    pa[0]=f.from.x+(w.x-f.from.x)*e0; pa[1]=f.from.y+(w.y-f.from.y)*e0; pa[2]=f.from.z+(w.z-f.from.z)*e0;

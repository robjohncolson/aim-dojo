...
(function buildRoad(){
  ...
  roadMat=new THREE.ShaderMaterial({ transparent:true, depthWrite:false, fog:false, blending:THREE.AdditiveBlending,
    ...
    fragmentShader:(ML_RIBBON?[
      ...
      '  float lane=f1, tide=tI.g'+(ROAD_GLYPH_PASS?', hold=floor(floor(tI.b*255.0+0.5)/2.0)':'')+';',      // hold is the GLYPH's alone on this presentation (a held cell carries ONE letter, at its head), so it leaves with the pass rather than leaning on the compiler to strip it. The boundary's own holdE is unpacked from tE below and is untouched. With ROAD_GLYPH_PASS true - which is EVERY wave-7 configuration - this emits the shipped line character for character

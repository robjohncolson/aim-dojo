(function(){ // GENERATED by tools/extract-inline.mjs — line-preserving JS mirror of aim-dojo-main.js, part 5/5. Line N here = aim-dojo-main.js line N. Do not edit.
...
let _konamiI=0, _konamiGrad=false;   // THE OLD CODE, trackpad form (user, 2026-08-22): W W S S A D A D · L-click · Space, spoken MID-LESSON through play itself — the taps stay taps, the click genuinely fires, and Space bows Moon Sensei out through the game's own graduation. Never persisted; taught for two seconds to everyone who graduates honestly (see setTrainPhase), so the secret propagates the old way
const _KONAMI=['KeyW','KeyW','KeyS','KeyS','KeyA','KeyD','KeyA','KeyD','MB0','Space'];
...
document.addEventListener('keydown',(e)=>{
  ...
  if(e.code==='Space'){
    if(want==='Space'){ _konamiI=0; _konamiGrad=true; e.preventDefault(); setTrainPhase(3);
...
document.addEventListener('mousedown',(e)=>{
  ...
  if(_KONAMI[_konamiI]==='MB0') _konamiI++;
  else _konamiI=0;
},true);
...
function flashTheme(){   // TUNE LIBRARY: ♪ name + song-colored flash + opening chord breath
  ...
  const ml=rl?'':ghostLocalMailLine();
  const vl=rl||vm||ml?'':(GH_SHARE?ghostVisitorLine():'');
  const dl=rl||ml?'':(vm||vl?'':(CFG.deal.on?dealLine():''));   // higher threshold lines suppress the deal sentence only

...
const CFG = {
  ...
  bow:{ on:true, holsterBeats:8, holsterMinSec:12, cancelGraceSec:1.5, riseBeatsPerOrb:1, riseBeats:2, ritBars:2, beatCapSec:0.75, mandalaSec:4, mandalaMaxDots:60, mandalaArcDeg:90, lineAtMandala:0.6, lineHoldSec:4, senseiSec:1.0 },   // holsterBeats/holsterMinSec = the two-clock trigger (the SLOWER wins, so a 20 BPM night can't bow on a short pause) · cancelGraceSec = the lossless undo window · riseBeatsPerOrb = Last Light stagger, riseBeats = each orb's own flight to the shell · ritBars = length of the closing ritardando · beatCapSec CAPS the beat length the whole ceremony is measured in, so the 20 BPM floor doesn't stretch the bow to a minute · mandalaSec = glyph fade-in→drift→dissolve · mandalaMaxDots = most recent arrivals kept (run-local, never persisted) · mandalaArcDeg = the ± arc a half-beat of arrival error is drawn across · lineAtMandala = fraction into the glyph at which THE line speaks · lineHoldSec/senseiSec = the line's hold and the ✦ bow before the pointer unlocks
...
function updateSky(dt){
  ...
  const tideBreath=(!reduceMotion && tideLive()) ? 1+(CFG.tide.tintExhale||0)*(_tideTint+=((tideMercy?1:0)-_tideTint)*Math.min(1,dt*2.2)) : 1;
...
function tideLive(){ return !!(CFG.tide && CFG.tide.on) && !trainMode; }
...
const _bow={stage:BOW.IDLE, t:0, idle:0, spb:0.75, rise:1, ritAt:0, manAt:0, lineAt:0, senseiAt:0, endAt:0, lined:false, bowed:false, orbs:[]};
...
function bowLive(){ return !!(CFG.bow && CFG.bow.on) && state.running && !trainMode && !templeActive && !bonusActive && _bowHits.length>0; }   // post-graduation, in a live run, outside the Temple — and only once the night has something to draw (a Mandala with no dots is not a ritual)
...
function bowCommit(){
  const B=CFG.bow;
  _bow.stage=BOW.GRACE; _bow.t=0; _bow.idle=0; _bow.lined=false; _bow.bowed=false; _bow.orbs.length=0;
  _bow.spb=Math.min(60/Math.max(20,state.bpm), B.beatCapSec||0.75);   // the ceremony's own beat length, capped so the 20 BPM floor doesn't stretch the bow to a minute
}
function bowEnterLast(){
  ...
  for(const tg of live){
    ...
    _bow.orbs.push({tg:tg, x0:p.x, y0:p.y, z0:p.z, ex:PLAYER_POS.x+dx/h*(h*3.2), ez:PLAYER_POS.z+dz/h*(h*3.2), ey:ROOM_BY*2.6, s0:tg.radius*tg.sc, at:_bow.t+_bow.orbs.length*stagger});   // outward along its OWN azimuth toward the sky shell, one per beat in spawn order
  }
...
function bowRiseOrbs(){
  for(let i=_bow.orbs.length-1;i>=0;i--){
    ...
    if(tg.mesh){
      if(!reduceMotion) tg.mesh.position.set(o.x0+(o.ex-o.x0)*e, o.y0+(o.ey-o.y0)*e, o.z0+(o.ez-o.z0)*e);   // reduceMotion: no travel, it just fades where it stands
      tg.mesh.scale.setScalar(Math.max(0.001, o.s0*(1-0.94*e)));   // shrinks to a point of light — the SHARED orb materials are never touched, so nothing else in the field changes
    }
    if(a>=1){ removeTarget(tg); swapRemove(_bow.orbs,i); }
  }
}
function bowEnterRit(){
  const B=CFG.bow; _bow.stage=BOW.RIT;
  const ritSec=Math.max(0.5,(B.ritBars||0)*4*_bow.spb);
...
function bowEnterHold(){
  ...
  const man=Math.max(0.1,B.mandalaSec||0), tail=Math.max(man, man*(B.lineAtMandala||0)+(B.lineHoldSec||0));
  ...
  const cv=bowCanvasEl(); if(cv) cv.style.display='block';
}
function bowUpdate(dt){
  ...
  bowDrawMandala((_bow.t-_bow.manAt)/Math.max(0.1,B.mandalaSec||0));
  if(!_bow.lined && _bow.t>=_bow.lineAt){ _bow.lined=true; try{ showGhostToast(bowSkyLine()); }catch(e){} }   // THE line — exactly one per Bow, the session's only new toast
...
function bowReset(){
  if(bowHolding()){
    for(const o of _bow.orbs) if(o.tg) removeTarget(o.tg);   // any Echo still mid-rise goes back to the pools
    _bow.orbs.length=0;
  ...
  _bow.stage=BOW.IDLE; _bow.t=0; _bow.idle=0; _bow.lined=false; _bow.bowed=false;
}
function bowFinish(){
...
function animate(frameNow){
  ...
  bowClock(dt);   // THE BOW: holster clock while idle, ceremony clock once committed. Zero work (one boolean) with the parcel off, in the trainer, in the Temple or before this run has scored.
  ...
  if(CFG.dolly && !reduceMotion && !templeActive){ const _t=state.t, _w=CFG.dollySpeed, D=0.017453293, _dm=dollyStrengthMul()*tideMul(CFG.tide.dollyLo);   // TIDES: the view wanders hardest at the crest and calms into the mercy bar (×1 with the parcel off)
  ...
  if(state.running && !templeActive){
    ...
    if(targets.length){
      ...
      const _tideW=tideMul(CFG.tide.brownianLo);   // TIDES: wander cap + juke sharpness breathe with the swell. grooveGlideSpeed is NOT scaled — the leadable glide is the sacred loop
...
function showGhostToast(txt){ if(!ghostToastEl) return; setText(ghostToastEl, txt); ghostToastEl.classList.remove('show'); void ghostToastEl.offsetWidth; ghostToastEl.classList.add('show'); }
...
function showPause(){
  ...
  if(beginLabel) beginLabel.textContent=T('resume','RESUME');
  else beginBtn.textContent='▶ '+T('resume','RESUME');

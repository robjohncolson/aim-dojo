...
function volleyNote(tg){
  ...
  if(CFG.voice.on && voiceLive() && voiceMuted()) return;           // THE LEAD INSTRUMENT: your clank still owns this beat. Raw boolean first, and the mute is the ONE thing that can silence a volley — filling the hole you made would undo it
...
function gradeRhythmHit(tg, point, atT, atBpm){
  ...
  if(good && CFG.chordVolley.on && !(tg.hpMax>1)) volleyNote(tg);   // CHORD VOLLEYS: only SCORING arrivals are offered to the tracker (a late GOOD, a clank, a whiff and a decoy all miss this line by construction), and the multi-hit tank is excluded whole — its hits are their own stated figure, not a volley voice. Raw boolean first so the parcel off costs one read and no call
